...
1.3 - Place rpm package on host inside folder named 'shared_files'
...
2.2 - Select 'Standard' install on first boot
       (Inside VM -> Software selection -> Minimal install -> Standard)

2.3 - Mount Guest Additions CD
       (VirtualBox VM window -> Devices -> Insert Guest Additions CD image...)

2.4 - Share folder from Step 1.3
       (VirtualBox VM window -> Devices -> Shared Folders -> Shared Folders Settings... -> Add new shared folder)
              - Folder Path: path to shared folder from Step 1.3
              - Folder Name: shared_files (or your preferred name)
              - Check 'Auto-mount' and 'Make Permanent'
       (RPM package path should exist as '/media/sf_shared_files/oracle-database-ee-19c-1.0-1.x86_64.rpm')
...
2.8 - cd into downloaded git repo using 'cd oracle_install'
